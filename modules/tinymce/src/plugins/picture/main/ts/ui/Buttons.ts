...
const register = (editor: Editor) => {
  ...
  editor.ui.registry.addToggleButton('picture', {
    icon: 'image',
    tooltip: 'Insert/edit picture',
    onAction: Dialog(editor).open,
    onSetup: (buttonApi) => editor.selection.selectorChangedWithUnbind('img:not([data-mce-object],[data-mce-placeholder]),figure.image,picture', buttonApi.setActive).unbind
  });

  editor.ui.registry.addMenuItem('picture', {
    icon: 'image',
    text: 'Picture...',
    onAction: Dialog(editor).open
  });

  editor.ui.registry.addContextMenu('picture', {
