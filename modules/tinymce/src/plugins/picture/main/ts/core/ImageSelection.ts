...
const getSelectedImage = (editor: Editor): HTMLElement => {
  ...
  if (figureElm) {
    return editor.dom.select('picture', figureElm)[0];
  }

  const pictureElm = editor.dom.getParent(imgElm, 'picture') as HTMLElement;
...
const syncSrcAttr = (editor: Editor, picture: HTMLElement) => {
